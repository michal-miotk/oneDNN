...
status_t get_devices(std::vector<cl_device_id> *devices,
    ...
    for (size_t i = 0; i < platforms.size(); ++i) {
        ...
        if (num_devices != 0) {
            ...
            OCL_CHECK(call_clGetDeviceIDs(platforms[i], device_type, num_devices,
                    &plat_devices[0], nullptr));

            // Use the devices for the requested vendor only.
            for (size_t j = 0; j < plat_devices.size(); ++j) {
                cl_uint v_id;
                OCL_CHECK(call_clGetDeviceInfo(plat_devices[j], CL_DEVICE_VENDOR_ID,
...
status_t get_device_index(size_t *index, cl_device_id device) {
    ...
    while (parent_device) {
        ...
        OCL_CHECK(call_clGetDeviceInfo(top_level_device, CL_DEVICE_PARENT_DEVICE,
...
status_t get_device_uuid(xpu::device_uuid_t &uuid, cl_device_id ocl_dev) {
    ...
    OCL_CHECK(call_clGetDeviceInfo(ocl_dev, CL_DEVICE_UUID_KHR, CL_UUID_SIZE_KHR,
...
status_t check_device(
    ...
    size_t dev_bytes;
    OCL_CHECK(
            call_clGetContextInfo(ctx, CL_CONTEXT_DEVICES, 0, nullptr, &dev_bytes));
